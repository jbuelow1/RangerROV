...
class GUI:
    ...
    def motorStats(self, Motors, Control, joystickXZ, joystickY, screen, AmpList, AmpPool, PWM):
        motorList = Motors.FindTotalSpeed(Motors, Control, joystickY, joystickXZ, AmpList, AmpPool, PWM)
        ...
        #PWMa = GUI.font.render("PWM Rotation: " + str(motorList[5]), 1, GUI.black, GUI.white)
        #PWMb = GUI.font.render("PWM Right Claw: " + str(motorList[6]), 1, GUI.black, GUI.white)
        #PWMc = GUI.font.render("PWM Left Claw: " + str(motorList[7]), 1, GUI.black, GUI.white)
        ...
        #screen.blit(PWMa, (1,250))
        #screen.blit(PWMb, (1,275))
        #screen.blit(PWMc, (1, 300))
...
class Motors:
    ...
    def findTimeHigh(self, Input):
        if Input == 0:
            ...
            TimeHigh = math.floor((3508*Input)+(2754*(1-Input)))
            return TimeHigh
        elif Input <0 and Input >= -1:
            Input *= (-1)
            TimeHigh = math.floor((2000*Input)+(2754*(1-Input)))
            return TimeHigh
        else:
    ...
    def FindYSpeed(self, Control, joystickY, Motors):
        frontMotor = (Control.YPosition(joystickY))+((Control.YTilt(joystickY)*-1))
        backMotor = (Control.YPosition(joystickY))-((Control.YTilt(joystickY)*-1))
    ...
    def FindTotalSpeed(self, Motors, Control, joystickY, joystickXZ, AmpList, AmpPool, PWM):
        ...
        if totalAmpsUsed > 2000000:
            ...
            return newTickList
        elif totalAmpsUsed <= 20000000:
            timesHigh.append(PWM.findPWM(PWM, Control, joystickXZ))
            timesHigh.append(totalAmpsUsed)
        #PWMStatList = PWM.findPWM(PWM, Con11trol, joystickY, joystickXZ)
        #for PWMnum in PWMStatList:
         #   timesHigh.append(PWMnum)
        if joystickXZ.get_button(0) == True:
            timesHigh[0] = 4096
            timesHigh[1] = 4096
            timesHigh[2] = 4096
            timesHigh[3] = 4096
...
class PWM:
    ...
    #beTick is the -1 to 1 that the contoller gives
    def controlToTicks(self, Input):
       
        if Input == 0:
           return(1535)
        elif Input > 0 and Input <= 1:
            TimeHigh = math.floor((4070*Input)+(1000*(1-Input)))
            return (TimeHigh)
        elif Input <0 and Input >= -1:
            Input *= (-1)
            TimeHigh = math.floor((1000*Input)+(4070*(1-Input)))
            return(TimeHigh)
        else:
            print("Error: Input was not between -1 and 1 as it was: " + str(joystickAxis))


        #Accept a single connection and make a file-like ob            return(1535)


    def findPWM(self, PWM, Control, joystickXZ):
        
        JoystickValue = Control.ClawPosition(joystickXZ)
        contolValues = PWM.controlToTicks(JoystickValue)
        return contolValues
...
class Camera:
    ...
    def serverStart(self):
        ...
        server_socket.listen(0)#ject out of it
        #connection = server_socket.accept()[0].makefile('rb')
        return server_socket
        
    def clientStart(self):
        client_socket = socket.socket()
        client_socket.connect(('196.168.0.6', 8000))
